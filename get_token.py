...
def background_token_refresher():
    """
    Luồng nền để tự động lọc và tải token.
    Sẽ chạy ngay khi khởi động và lặp lại mỗi 8 giờ.
    """
    global task_status
    while True:
        # Lọc và tải token ngay lập tức khi luồng được khởi động
        task_status = "Đang chạy quá trình lấy và tải token..."
        print("\n--- Bắt đầu quá trình lấy và tải token ---")
        tokens = get_tokens_and_upload()
        if tokens:
            if upload_tokens_to_api(tokens):
                task_status = f"Cập nhật token thành công lúc {time.strftime('%Y-%m-%d %H:%M:%S')}."
            else:
                task_status = f"Cập nhật token thất bại lúc {time.strftime('%Y-%m-%d %H:%M:%S')}."
        else:
            task_status = f"Không có token nào được lấy thành công lúc {time.strftime('%Y-%m-%d %H:%M:%S')}."

        print(f"--- Kết thúc quá trình. Chờ 8 tiếng để lọc lại token (lần tiếp theo lúc {time.strftime('%Y-%m-%d %H:%M:%S', time.time() + 8 * 3600)}) ---")
...
def home():
    return jsonify({
        ...
        "next_run_estimate": f"Lần chạy tiếp theo dự kiến khoảng {time.strftime('%H:%M:%S', time.time() + 8 * 3600)} sau lần chạy cuối cùng thành công."
    })
...
def run_now():
    global task_status
    # Khởi động một luồng mới để chạy tác vụ thủ công
    # Để tránh chạy chồng chéo với tác vụ định kỳ, bạn có thể thêm logic khóa (lock) nếu cần
...
def get_tokens_and_upload_single_run():
    ...
    if tokens:
        if upload_tokens_to_api(tokens):
            task_status = f"Chạy thủ công thành công lúc {time.strftime('%Y-%m-%d %H:%M:%S')}."
        else:
            task_status = f"Chạy thủ công thất bại lúc {time.strftime('%Y-%m-%d %H:%M:%S')}."
    else:
        task_status = f"Chạy thủ công không có token nào được lấy thành công lúc {time.strftime('%Y-%m-%d %H:%M:%S')}."
...
if __name__ == "__main__":
    ...
    # Môi trường Render sẽ tự động đặt biến môi trường PORT
